...
# Simple test: delegate MSI to S-mode and verify it arrives in S-mode
...
_start:
    ...
    # Delegate MSI to S-mode via mideleg
    li      t0, 0x08                # Bit 3 = MSI delegation
    csrw    mideleg, t0

    # Enable software interrupt in sie
    li      t0, 0x02                # SSIE (bit 1)
    csrs    sie, t0
...
s_mode_code:
    ...
    # Trigger software interrupt via MSIP
...
got_interrupt:
    # Verify it was SSI (cause 1)
    la      t0, s_cause
    lw      t1, 0(t0)
    li      t2, 0x80000001          # SSI cause
    bne     t1, t2, test_fail
...
.align 4
m_trap_handler:
    # If we're here, delegation failed
    li      a0, 3
    ebreak
